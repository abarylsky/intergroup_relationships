...
corr.test(Goldberg_gav_sm %>% filter(Partner == 1) %>% select(h12social, h12selfdir, h12conform))
```
...
Out of the `r dim(Goldberg_gav_sm)[1]` participants, `r dim(filter(Goldberg_gav_sm, gender == 1))[1]` are female and `r dim(filter(Goldberg_gav_sm, gender == 2))[1]` are male. In this subset of participants, 
`r pluck(sexor_counts, 4, 1)`% (n = `r pluck(sexor_counts, 3, 1)`) identify as `r pluck(sexor_counts, 2, 1)`,  
`r pluck(sexor_counts, 4, 2)`% (n = `r pluck(sexor_counts, 3, 2)`) identify as `r pluck(sexor_counts, 2, 2)`, 
`r pluck(sexor_counts, 4, 3)`% (n = `r pluck(sexor_counts, 3, 3)`) identify as `r pluck(sexor_counts, 2, 3)`,  
`r pluck(sexor_counts, 4, 4)`% (n = `r pluck(sexor_counts, 3, 4)`) identify as `r pluck(sexor_counts, 2, 4)`, 
`r pluck(sexor_counts, 4, 5)`% (n = `r pluck(sexor_counts, 3, 5)`) identify as `r pluck(sexor_counts, 2, 5)`, 
`r pluck(sexor_counts, 4, 6)`% (n = `r pluck(sexor_counts, 3, 6)`) identify as `r pluck(sexor_counts, 2, 6)`, and 
`r pluck(sexor_counts, 4, 7)`% (n = `r pluck(sexor_counts, 3, 7)`) identify as `r pluck(sexor_counts, 2, 7)`. 
